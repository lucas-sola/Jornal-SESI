document.addEventListener('DOMContentLoaded', async () => {
    ...
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-color); font-size: 1.1rem;"><div class="spinner" style="margin: 0 auto 15px;"></div>Carregando esportes...</div>';
    ...
    try {
        ...
        // Filtrar matérias que pertencem à categoria/tema de Esportes
        let esportes = dados.filter(p => p.tema_principal_nome === 'Esportes');

        // Fallback caso não haja nenhuma cadastrada
        ...
        const selecionadas = esportes.slice(0, 6);
...
function criarCardElement(noticia) {
    ...
    const imgPadrao = 'src/images/foto-sesi.jpg';
    const imgSrc = resolverImagemPublicacao(noticia.imagem_destaque, imgPadrao);

    // Como é a página de Esportes, marcamos como "Esportes" ou mantemos o tema original
    const tag = noticia.tema_principal_nome || 'Esportes'; 

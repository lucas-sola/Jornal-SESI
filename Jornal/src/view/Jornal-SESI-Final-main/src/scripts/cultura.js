document.addEventListener('DOMContentLoaded', async () => {
    ...
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-color); font-size: 1.1rem;"><div class="spinner" style="margin: 0 auto 15px;"></div>Carregando cultura...</div>';
    ...
    try {
        ...
        // Filtro para Cultura: matérias relacionadas a depoimento pessoal (7), manifestos (4) ou temas de educação/sociedade
        let cultura = dados.filter(p => [4, 7].includes(p.genero_id) || p.tema_principal_id === 4);
        ...
        const selecionadas = cultura.slice(0, 6);

document.addEventListener('DOMContentLoaded', async () => {
    ...
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-color); font-size: 1.1rem;"><div class="spinner" style="margin: 0 auto 15px;"></div>Carregando ciências...</div>';
    ...
    try {
        ...
        // Filtro para Ciências: Avanço Tecnológico (3) ou Saúde (7) ou Ciência (8)
        let ciencias = dados.filter(p => [3, 7, 8].includes(p.tema_principal_id));

        // Se houver poucas matérias no filtro, pegamos qualquer uma do banco para manter a página preenchida
        if (ciencias.length < 3) {
            ciencias = [...dados];
        }
        ...
        const selecionadas = ciencias.slice(0, 6);
        ...
        console.error('Erro ao carregar ciências:', error);
...
function criarCardElement(noticia) {
    ...
    const imgPadrao = 'src/images/imagens-links/ministerio-da-ciencia-e-tecnologia.jpg';
    const imgSrc = resolverImagemPublicacao(noticia.imagem_destaque, imgPadrao);

    const tag = noticia.tema_principal_nome || noticia.genero_nome || 'Ciência';

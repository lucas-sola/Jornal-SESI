...
// Conexão do Formulário do Footer
document.addEventListener('DOMContentLoaded', () => {
    const formFooter = document.querySelector('#formEnvioTexto');
    if (formFooter) {
        formFooter.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = formFooter.querySelector('button');
            const successMsg = document.querySelector('#footerSuccess');

            const dados = {
                nome: document.querySelector('#footerNome').value,
                email: document.querySelector('#footerEmail').value,
                categoria: document.querySelector('#footerCategoria').value
            };
            ...
            try {
                const response = await fetch('/api/autores', { // Exemplo: cadastrando como autor provisório
                    method: 'POST',
                ...
                console.error('Erro ao enviar rodapé:', error);
            } finally {
                btn.disabled = false;
                btn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Enviar texto';
            }
        });
    }
});
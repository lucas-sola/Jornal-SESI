document.addEventListener("DOMContentLoaded", async () => {
  ...
  grid.innerHTML =
    '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-color); font-size: 1.1rem;"><div class="spinner" style="margin: 0 auto 15px;"></div>Carregando notícias...</div>';
  ...
  try {
    ...
    if (!dados || dados.length === 0) {
      grid.innerHTML =
        '<p style="grid-column: 1/-1; text-align: center; padding: 20px;">Nenhuma matéria encontrada no banco.</p>';
      return;
    }

    // Para Notícias, queremos temas como Conflitos Bélicos (1), Legislações (2), Violência/Segurança (4), Eleições (6), Vício em Jogos (9)
    // Mas para não deixar a página vazia ou repetitiva, pegamos do banco e filtramos
    let noticias = dados.filter((p) =>
      [1, 2, 4, 6, 9].includes(p.tema_principal_id),
    );

    // Se houver poucas, pegamos qualquer uma do banco
    ...
    // Limitar a no máximo 6 notícias para a página
    const selecionadas = noticias.slice(0, 6);
    ...
    // Configurar animação e curtidas
    iniciarAnimacaoECurtidas();
  } catch (error) {
    console.error("Erro ao carregar notícias:", error);
    grid.innerHTML =
      '<p style="grid-column: 1/-1; text-align: center; padding: 20px; color: var(--primary-color);">Erro ao carregar matérias do banco de dados.</p>';
  }
});
...
function criarCardElement(noticia) {
  ...
  // Determinar imagem padrão ou real (com fallback aplicado depois via onerror)
  const imgIndex = (noticia.id % 2) + 1;
  const extensaoPadrao = imgIndex === 1 ? "png" : "jpg";
  const imgPadrao = `src/images/noticias/noticia-${imgIndex}.${extensaoPadrao}`;
  const imgSrc = resolverImagemPublicacao(noticia.imagem_destaque, imgPadrao);

  const tag = noticia.tema_principal_nome || noticia.genero_nome || "Notícias";
  const titulo = noticia.titulo;
  const resumo =
    noticia.resumo ||
    (noticia.conteudo ? noticia.conteudo.substring(0, 160) + "..." : "");
  const autor = noticia.autor_nome || "Equipe SESI";
  const dataFormated = formatarData(
    noticia.data_publicacao || noticia.data_criacao,
  );
  ...
  // Salvar o ID da publicação no elemento para a lógica de curtidas
...
function iniciarAnimacaoECurtidas() {
  ...
  // 3. Filtro de pesquisa integrado ao cabeçalho para cards dinâmicos
  ...
  inputsPesquisa.forEach((input) => {
    input.addEventListener("input", (e) => {
      ...
      cards.forEach((card) => {
        ...
        const resumo = card
          .querySelector(".post-card-excerpt")
          .textContent.toLowerCase();
        const tag = card
          .querySelector(".post-card-tag")
          .textContent.toLowerCase();

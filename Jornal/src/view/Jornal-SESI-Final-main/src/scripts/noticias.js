document.addEventListener('DOMContentLoaded', async () => {
    ...
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-color); font-size: 1.1rem;"><div class="spinner" style="margin: 0 auto 15px;"></div>Carregando notícias...</div>';
    ...
    try {
        ...
        let noticias = dados.filter(p => [1, 2, 4, 6, 9].includes(p.tema_principal_id));
...
function criarCardElement(noticia) {
    ...
    // Determinar imagem padrão ou real
    let imgSrc = 'src/images/noticias/noticia-1.png';
    if (noticia.imagem_destaque) {
        ...
        // Imagens aleatórias estéticas dependendo do ID para dar variedade
        const imgIndex = (noticia.id % 2) + 1;
        imgSrc = `src/images/noticias/noticia-${imgIndex}.png`;
    }
